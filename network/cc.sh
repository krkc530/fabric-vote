...
# chaincode version upgrade 
# docker exec cli peer chaincode upgrade -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CCNAME -v $VERSION -p ${CC_SRC_PATH} -c '{"Args":[]}'

# chaincode test
docker exec cli peer chaincode invoke -C $CHANNEL_NAME -n $CCNAME -c '{"Args":["upload", "testKey", "testTag", "testBytes"]}' 
sleep 3
